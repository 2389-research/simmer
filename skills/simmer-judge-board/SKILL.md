...
description: >
  Do not invoke directly — dispatched by the simmer orchestrator when JUDGE_MODE is board. Runs a
  panel of perspective-diverse judges to produce consensus scores and a single ASI; drop-in
  replacement for simmer-judge used for complex or plateaued artifacts.
---
...
See [references/judge-primitives.md](references/judge-primitives.md) for the Judge Primitive Library, Example Compositions, and Custom Judge Panels.
